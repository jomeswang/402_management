<template>
  <div>
    <div class="filter-container">
      ...
      <el-input
        v-model="couponFilter.couponName"
        clearable
        placeholder="优惠券名称"
        style="width: 200px;"
        class="filter-item"
    ...
    <el-table :data="userCoupon" border fit highlight-current-row style="width: 100%;">
      <el-table-column
        :label="$t('userCoupon.couponId')"
        prop="couponId"
        align="center"
        width="200"
      >
  ...
  methods: {
    handleFilter(value) {
      ...
      if (this.couponFilter.couponName !== '' && this.couponFilter.userName !== '' && typeof (this.couponFilter.userName) !== undefined) {
        ...
        const arr2 = []
        arr1.forEach(item => {
          if (item.userName.indexOf(this.couponFilter.userName) !== -1) { arr2.push(item) }
        })
        let arr3 = []
        if (this.couponFilter.status !== '' && typeof (this.couponFilter.status) !== undefined) {
        ...
        const arr1 = []
        this.listCopy.forEach(item => {
          if (item.userName.indexOf(this.couponFilter.userName) !== -1) { arr1.push(item) }
        })
        let arr2 = []
        if (this.couponFilter.status !== '' && typeof (this.couponFilter.status) !== undefined) {
          arr2 = arr1.filter(item => {
