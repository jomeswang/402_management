<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      />
      <el-input
        v-model="listQuery.order_name"
        clearable
        :placeholder="$t('table.order_name')"
        style="width: 200px;"
        class="filter-item"
        @clear="handleFilter"
        @keyup.enter.native="handleFilter"
      ...
      <el-checkbox
        v-model="showCreditId"
        class="filter-item"
        style="margin-left:15px;"
        @change="tableKey=tableKey>100?0:tableKey+1"
      >
        订单交易时间
      </el-checkbox>
    </div>
    ...
    >
      ...
      >
        ...
        <template slot-scope="{row}">
          <span
            class="link-type"
            @click="showDetail(row, row.order_id)"
          >{{ row.order_id }}</span>
        </template>
      ...
      <el-table-column
        :label="$t('table.order_phone_number')"
        width="150px"
        align="center"
      >
        <template slot-scope="{row}">
      ...
      >
        <template slot-scope="{row,$index}">
          <el-button
            v-if="row.status==='待确认'?false:row.status==='已取消'?false:true"
            type="success"
            size="mini"
          ...
          <el-button
            v-if="row.status==='待确认'?true:false"
            type="primary"
            size="mini"
    ...
    >
      ...
      >
        <span style="margin-left: 50px;margin-right: 30px;margin-top: 10px">退款金额</span>
        <el-input
          v-model="handleRoomStatus.price"
          style="width: 50%"
        />
      </div>
      <div
        ...
        style="display: flex; margin-top: 30px"
      >
        <span style="margin-left: 50px;margin-right: 30px;margin-top: 10px">退押金额</span>
        <el-input
          v-model="handleRoomStatus.deposit"
          style="width: 50%"
        />
      </div>
